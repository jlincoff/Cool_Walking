# This script sets up and performs a temperature replica exchange simulation.
...
# Input pdb file, specify force field, solvate peptide.
...
beta = Quantity(-1.0 / (1.3806488 * 6.022 / 1000),1/kilojoules_per_mole) # Used to calculate boltzmann weights.

# Array of temperatures for replicas, in Kelvin.
...
for i in range(numtemps):
    ...
    vars()['sys'].addForce(AndersenThermostat(temperatures[i]*kelvin, 25/picosecond))
    ...
    # Construct simulation, add to list, minimize, and set velocities.
    sim = 't' + repr(i) + '_sim'
    ...
    sim_objs[i].context.setVelocitiesToTemperature(temperatures[i]*kelvin)

    # Add reporters. In this case reporters are only added to the low temperature replica.
    if i == 0:
        sim_objs[i].reporters.append(PDBReporter('TREx_alanine.pdb',100))
        sim_objs[i].reporters.append(StateDataReporter('TREx_alanine.txt',100,step = True, potentialEnergy = True, totalEnergy = True, temperature = True))
...
for steps in range(50000):
    ...
    for j in range(len(set)):
        ...
        acc = math.exp((beta/temperatures[pair - 1] - beta/temperatures[pair])*energy1)
        ...
        if acc > random.uniform(0.0,1.0):
            ...
            sim_objs[pair - 1].context.setVelocities(state2.getVelocities()*math.sqrt(temperatures[pair - 1]/temperatures[pair]))
            sim_objs[pair].context.setVelocities(state1.getVelocities()*math.sqrt(temperatures[pair]/temperatures[pair-1]))
